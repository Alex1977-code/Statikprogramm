...
def test_auf_netz():
    ...
    check("der Sweep bleibt aus, auch mit VQ83 in der Vorgabe", n.sweep is False)
    n_s = ea.auf_netz(Netzeinstellungen(sweep=True), {"tet4"})
    check("ein eingeschalteter Sweep bleibt an", n_s.sweep is True and n_s.ordnung == 1)
...
def test_maske_elementwahl():
    ...
    check("Übernehmen: ordnung 2, Pyramiden an, Sweep bleibt aus",
          w.model.netz.ordnung == 2 and w.model.netz.pyramiden and not w.model.netz.sweep)

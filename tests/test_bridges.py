...
def test_reihe_ohne_verlangten_nachweis():
    ...
    am Beispiel gate mit drei Stellungen: ohne Argument je Stellung
    eta 0.000, nicht nachgewiesen, keine Warnung, ok; mit nachweise=True
    eta 0.515. Ebenso ohne Warnung: alle Staebe design = False."""
    ...
    def ohne_nachweis(vorsatz, u):
        ...
        check(f"{vorsatz}: kein Stab nachgewiesen, keine Warnung, eta 0, ok",
              len(erg) == 3 and not u.fehlerhaft
              and all(not e.nachgewiesen and e.warnungen == [] and e.eta == 0.0 and e.ok
                      for e in erg),
              str([(round(e.eta, 3), e.nachgewiesen, len(e.warnungen), e.ok) for e in erg]))
        check(f"{vorsatz}: kurztext und Bericht nennen 'eta = 0.000'",
              kurz.startswith("eta = 0.000, maßgebend ")
              and "Umhüllende: eta = 0.000, maßgebend in " in b
              and "nicht bestimmt" not in kurz + b and "NICHT VOLLSTÄNDIG" not in kurz + b,
    ...
    check("Gegenfall rechnen(nachweise=True): alle Stellungen nachgewiesen, eta > 0",
          ...
          and not u.kurztext().startswith("eta = 0.000"),

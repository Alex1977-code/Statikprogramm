...
(eigener Schalter ``Netzeinstellungen.sweep``) macht immer hex8/pent6.
    ...
    if tet == "tet4":
        neu = ("Neue hex8 entstehen in abgebildeten Sechsflächnern (sechs Vierecke, acht Ecken, "
               "gerade Kanten)" + (" und im Sweep." if sweep else
                                   "; im Sweep nur mit dem Schalter „Sechsflächner sweepen“ (aus)."))
    else:
        neu = ("Neue hex8 entstehen nur im Sweep - " + (
            "der Schalter „Sechsflächner sweepen“ ist an." if sweep else
            "der Schalter „Sechsflächner sweepen“ (Netzeinstellungen) ist aus, und die "
...
def vorschau(wahl, sweep: bool = False) -> list:
    ...
    zeilen = [f"frei vernetzte Körper: {tet}"
              ...
              "gesweepte Körper: " + ("hex8 und pent6 (VQ83)" if sweep else
                                      "keine - „Sechsflächner sweepen“ ist aus"),

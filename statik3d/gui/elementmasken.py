...
class Elementwahl(msk.Maske):
    ...
    def __init__(self, netz, parent=None):
        self.sweep = bool(getattr(netz, "sweep", False))
        super().__init__(
        ...
        for key, gruppe, text in ea.HAKEN:
            ...
            self.haken[key] = cb
            if key == "hex20":
                # Der Sweep-Schalter steht daneben, lesbar, nicht verstellbar:
                # die Elementwahl darf ihn nicht einschalten (Auftrag Paket E)
                ...
                self.lbl_sweep.setText(
                    "„Sechsflächner sweepen“ (Netzeinstellungen): "
                    + ("an - gesweepte Körper werden hex8/pent6." if self.sweep else
                       "aus - hex8 entstehen nur dort, wo er an ist; diese Wahl schaltet ihn "
                       "nicht ein."))
